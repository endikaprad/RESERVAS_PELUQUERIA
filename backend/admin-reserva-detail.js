...
(function initReservaDetail() {
    ...
    <div class="rd-drawer" id="rd-drawer" role="dialog" aria-modal="true" aria-label="Detalle de reserva">
        ...
        <!-- TAB: RESERVA ────────────────────────────────────── -->
        <div class="rd-body" id="rd-pane-reserva">
            ...
            <div id="rd-stats-content" style="display:none;">
                ...
                <div class="rd-section">
                    ...
                    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;">
                        <div class="rd-stat-kpi rd-stat-accent">
                            <div class="rd-stat-val" id="cs-visitas">0</div>
                            <div class="rd-stat-lbl">Visitas</div>
                        </div>
                        <div class="rd-stat-kpi">
                            <div class="rd-stat-val" id="cs-gasto">0 €</div>
                            <div class="rd-stat-lbl">Gasto total</div>
                        </div>
                        <div class="rd-stat-kpi">
                            <div class="rd-stat-val" id="cs-ticket">0 €</div>
                            <div class="rd-stat-lbl">Ticket medio</div>
                        </div>
                    </div>
                    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:8px;">
                        <div class="rd-stat-kpi">
                            <div class="rd-stat-val" id="cs-6m">0</div>
                            <div class="rd-stat-lbl">Últ. 6 meses</div>
                        </div>
                        <div class="rd-stat-kpi">
                            <div class="rd-stat-val" id="cs-cancel">0</div>
                            <div class="rd-stat-lbl">Cancelaciones</div>
                        </div>
                        <div class="rd-stat-kpi">
                            <div class="rd-stat-val" id="cs-freq">—</div>
                            <div class="rd-stat-lbl">Frec. (sem)</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="rd-section" id="cs-nivel-section">
                    ...
                    <div class="rd-cliente-card" style="margin-bottom:.75rem;">
                        <div class="rd-avatar" id="cs-nivel-icon" style="font-size:1rem;">—</div>
                        <div>
                            <div style="font-size:.9rem;font-weight:600;" id="cs-nivel-nombre">—</div>
                            <div style="font-size:.75rem;color:#7a7880;" id="cs-nivel-sub">—</div>
                        </div>
                    </div>
                    <div id="cs-progreso-wrap" style="display:none;">
                        <div style="display:flex;justify-content:space-between;font-size:.72rem;color:#7a7880;margin-bottom:5px;">
                            <span id="cs-prog-label">Siguiente</span>
                            <span id="cs-prog-faltan">—</span>
                        </div>
                        <div style="height:6px;background:#1c1c26;border-radius:3px;overflow:hidden;border:1px solid #252530;">
                            <div id="cs-prog-fill" style="height:100%;border-radius:3px;background:#ef9f27;width:0%;transition:width 1s ease .2s;"></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="rd-section" style="border-bottom:none;" id="cs-nota-section">
                    ...
                    <div id="cs-nota-edit-wrap" style="display:none;margin-top:.5rem;">
                        <textarea id="cs-nota-input" rows="4"
                            style="width:100%;background:#18181f;border:1px solid #252530;border-radius:8px;padding:.75rem;color:#f0ece3;font-family:'DM Sans',sans-serif;font-size:.85rem;resize:vertical;"
                            placeholder="Preferencias de corte, observaciones del cliente…"></textarea>
                        <div style="display:flex;gap:.5rem;margin-top:.5rem;">
                            <button onclick="rdNotaCancel()" style="flex:1;padding:.6rem;border-radius:6px;background:transparent;border:1px solid #252530;color:#7a7880;font-family:'DM Sans',sans-serif;font-size:.72rem;font-weight:600;letter-spacing:.08em;text-transform:uppercase;cursor:pointer;">Cancelar</button>
                            <button onclick="rdNotaSave()" id="cs-nota-save-btn" style="flex:2;padding:.6rem;border-radius:6px;background:linear-gradient(135deg,#d42b2b,#a81e1e);border:none;color:#fff;font-family:'DM Sans',sans-serif;font-size:.72rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;cursor:pointer;">Guardar nota</button>
                        </div>
                    </div>
                    <button onclick="rdNotaEdit()" id="cs-nota-edit-btn" style="margin-top:.6rem;width:100%;font-size:.72rem;padding:.55rem;color:#7a7880;border:1px dashed #252530;border-radius:7px;background:transparent;cursor:pointer;font-family:'DM Sans',sans-serif;letter-spacing:.06em;text-transform:uppercase;">✎ Editar nota</button>
                </div>
    ...
    const ESTADO_LABELS = {
        ...
        denegada: '✕ Denegada', cancelada: '✕ Cancelada',
    ...
    async function loadClientStats() {
        ...
        try {
            ...
            document.getElementById('rd-stats-loading').innerHTML =
                '<span style="color:#d42b2b;font-size:.8rem;">Error al cargar datos del cliente.</span>';
        }
    }

    function renderClientStats(d) {
        document.getElementById('rd-stats-loading').style.display = 'none';
        document.getElementById('rd-stats-content').style.display = 'block';
        ...
        if (nivelNom) {
            ...
            nivelNom.style.color = nivel.color_text || '#f0ece3';
        }
        if (nivelSub) {
            nivelSub.textContent = d.cliente_desde ? 'Cliente desde ' + d.cliente_desde : '';
        }
        const prog = d.progreso_siguiente;
        ...
        // Gasto por servicio
        const gastoEl = document.getElementById('cs-gasto-list');
        if (gastoEl) {
            if (d.gasto_por_servicio && d.gasto_por_servicio.length) {
                gastoEl.innerHTML = d.gasto_por_servicio.map(s => `
        ...
        // Insights
        const insEl = document.getElementById('cs-insights-list');
        if (insEl) {
        ...
        // Nota interna
        currentNota = d.nota_interna || '';
        const notaView = document.getElementById('cs-nota-view');
        if (notaView) {
            notaView.textContent = currentNota || 'Sin nota interna.';
            notaView.style.fontStyle = currentNota ? 'italic' : 'normal';
        }
        const notaInput = document.getElementById('cs-nota-input');
    ...
    function parseHistorial(motivoCambio, estado, ronda, fechaOriginal, horaOriginal, nuevaFechaProp, nuevaHoraProp) {
        ...
        const origSlotStr = (fechaOriginal && horaOriginal) ? formatFecha(fechaOriginal) + ' · ' + normHora(horaOriginal) : '—';
        const barberoSlotsFromLog = {};
        const clientSlots = {};
        ...
        partes.forEach(p => {
            ...
            if (!isClientEntry) {
                ...
                if (propMatch) barberoSlotsFromLog[barberoRondaCounter] = formatFecha(propMatch[1]) + ' · ' + propMatch[2];
            } else {
        ...
        const latestStoredSlot = (nuevaFechaProp && nuevaHoraProp) ? formatFecha(nuevaFechaProp) + ' · ' + normHora(nuevaHoraProp) : null;
        ...
        partes.forEach(p => {
            ...
            if (!isClientEntry) {
                barberoIdx++;
                const slotOrigen = barberoIdx === 1 ? origSlotStr : (clientSlots[barberoIdx - 1] || null);
                const motivoLimpio = p.replace(/\s*\[propuesta:[^\]]+\]/gi, '').trim();
                items.push({ tipo: 'barbero', icono: '⇄', titulo: 'Barbero propuso cambio', detalle: motivoLimpio, rondaLabel: 'Ronda ' + barberoIdx, slotOrigen, slotDestino: barberoSlotsFromLog[barberoIdx] || null });
            } else {
                const roundMatch = p.match(/ronda\s*(\d+)/i);
                const clientRonda = roundMatch ? parseInt(roundMatch[1]) : barberoIdx;
                items.push({ tipo: 'cliente', icono: '↩', titulo: 'Cliente propuso alternativa', detalle: 'El cliente no pudo en el horario propuesto.', rondaLabel: 'Ronda ' + clientRonda, slotOrigen: barberoSlotsFromLog[clientRonda] || null, slotDestino: clientSlots[clientRonda] || null });
            }
        });
        ...
        if (esNegociacionFinalizada && (ronda > 0 || partes.length > 0)) {
            if (estado === 'aceptada') {
                const fechaFmt = (fechaOriginal && horaOriginal) ? formatFecha(fechaOriginal) + ' · ' + normHora(horaOriginal) : '—';
                items.push({ tipo: 'final', icono: '✓', titulo: 'Negociación finalizada — Cita confirmada', detalle: '', rondaLabel: null, slotOrigen: null, slotDestino: fechaFmt });
            } else {
                items.push({ tipo: 'cancelled', icono: '✕', titulo: 'Negociación finalizada — Cita denegada', detalle: 'No se llegó a un acuerdo.', rondaLabel: null, slotOrigen: null, slotDestino: null });
            }
        }
        return items;
    }
    ...
    window.rdShowReschedule = function () {
        ...
        rdCalDate = new Date(); rdRenderCal();
        const body = document.getElementById('rd-pane-reserva');
        setTimeout(() => { body.scrollTop = body.scrollHeight; }, 50);
    };
    ...
    function extractFromRow(row) {
        try {
            ...
            return {
                ...
                servicio, duracion, precio, barbero,
