<?php
// ============================================================
//  POST /api/booking.php  — con Google Calendar (ICS + Schema.org)
// ============================================================
...
if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $fecha)) jsonError('Formato de fecha inválido');
if (!preg_match('/^\d{2}:\d{2}$/', $hora))         jsonError('Formato de hora inválido');
...
if ($fechaHora <= $ahora) jsonError('No puedes reservar en el pasado ni en la hora actual');
...
try {
    ...
    $cleanUp = $db->prepare(
        "DELETE FROM reservas WHERE barbero_id = ? AND fecha = ? AND hora = ?
    ...
    if ((int)$checkSlot->fetchColumn() > 0) jsonError('Ese horario ya está reservado. Por favor elige otro.', 409);

    // ── Auto-aceptar ─────────────────────────────────────────
    $autoAceptar = 'no'; $autoAceptarHasta = '';
    try {
        $cfgStmt = $db->prepare("SELECT clave, valor FROM configuracion WHERE clave IN ('auto_aceptar','auto_aceptar_hasta')");
        $cfgStmt->execute();
    ...
    } catch (Exception $e) { $autoAceptar = 'no'; }

    $estadoFinal = 'pendiente';
    if ($autoAceptar !== 'no' && $autoAceptarHasta !== '') {
        if ($ahora->format('Y-m-d') <= $autoAceptarHasta) $estadoFinal = 'aceptada';
    ...
    $insert->execute([$servicio, $barbero, $fecha, $hora, $nombre, $telefono, $email, $notas, $estadoFinal, $token]);
    $id = $db->lastInsertId();
    ...
    $fechaFormateada = $dias[$dt->format('w')] . ', ' . $dt->format('j') . ' de ' . $meses[(int)$dt->format('n')-1] . ' de ' . $dt->format('Y');

    $baseUrl      = 'https://pradopeluqueria.infinityfree.me';
    $durMin       = parseDuracionMinutos($servicioRow['duracion'] ?? '30 min');
    ...
    // ── ICS (adjunto para Gmail) ──────────────────────────────
    $icsUid     = 'reserva-' . $id . '@pradobarber.es';
    $icsContent = buildIcsContent($fecha, $hora, $durMin, $servicioRow['nombre'], $barberoRow['nombre'], $notas, $icsUid);

    // ── Schema.org (banner de Gmail) ─────────────────────────
    $schemaScript = buildSchemaOrgScript($fecha, $hora, $durMin, $servicioRow['nombre'], $barberoRow['nombre']);

    // ── Google Calendar URL (botón fallback) ─────────────────
    $gcalUrl   = buildGCalUrl($fecha, $hora, $durMin, $servicioRow['nombre'], $barberoRow['nombre'], $notas);
    ...
    $cancelBox = "
      <div style='background:#18181f;border:1px solid #252530;border-radius:8px;padding:16px;margin-bottom:24px;text-align:center;'>
        <p style='color:#7a7880;font-size:13px;margin:0 0 12px;'>¿Necesitas cancelar tu reserva?</p>
        <a href='{$urlCancelar}' style='display:inline-block;background:#374151;color:#f0ece3;text-decoration:none;
           padding:10px 24px;border-radius:6px;font-size:13px;font-weight:600;letter-spacing:0.08em;text-transform:uppercase;border:1px solid #4b5563;'>
          Cancelar reserva
        </a>
        <p style='color:#52525b;font-size:11px;margin:10px 0 0;'>Solo disponible hasta las 23:59 del día anterior.</p>
      </div>";
...
<body style='margin:0;padding:0;background:#09080f;font-family:Arial,sans-serif;'>
  <div style='max-width:560px;margin:0 auto;background:#111119;border:1px solid #252530;border-radius:12px;overflow:hidden;'>
    <div style='background:#22c55e;padding:24px 32px;'>
      ...
      <p style='margin:6px 0 0;color:rgba(255,255,255,0.8);font-size:14px;'>Prado Barber Co. — Auto-aceptación activa</p>
    </div>
    <div style='padding:32px;'>
      ...
      <p style='color:#7a7880;font-size:12px;text-align:center;'>Reserva #{$id} &middot; <a href='{$baseUrl}/backend/admin.php' style='color:#22c55e;'>Ver en el panel</a></p>
    </div>
  </div>
</body></html>";
    } else {
...
<body style='margin:0;padding:0;background:#09080f;font-family:Arial,sans-serif;'>
  <div style='max-width:560px;margin:0 auto;background:#111119;border:1px solid #252530;border-radius:12px;overflow:hidden;'>
        ...
        <tr>
          <td style='padding-right:8px;'>
            <a href='{$urlAceptar}' style='display:block;background:#22c55e;color:#fff;text-decoration:none;
               padding:14px;border-radius:6px;font-size:14px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;text-align:center;'>Aceptar reserva</a>
          </td>
          <td style='padding-left:8px;'>
            <a href='{$urlDenegar}' style='display:block;background:#d42b2b;color:#fff;text-decoration:none;
               padding:14px;border-radius:6px;font-size:14px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;text-align:center;'>Denegar reserva</a>
          </td>
        </tr>
      </table>
    ...
    // ── EMAIL AL CLIENTE ──────────────────────────────────────
    if ($estadoFinal === 'aceptada') {
        $htmlCliente = "<!DOCTYPE html><html lang='es'><head><meta charset='UTF-8'>
{$schemaScript}
</head>
<body style='margin:0;padding:0;background:#09080f;font-family:Arial,sans-serif;'>
  <div style='max-width:560px;margin:0 auto;background:#111119;border:1px solid #252530;border-radius:12px;overflow:hidden;'>
...
<body style='margin:0;padding:0;background:#09080f;font-family:Arial,sans-serif;'>
  <div style='max-width:560px;margin:0 auto;background:#111119;border:1px solid #252530;border-radius:12px;overflow:hidden;'>
    ...
    <div style='padding:32px;'>
      <p style='color:#f0ece3;font-size:15px;margin-bottom:24px;'>
        Hola <strong>".htmlspecialchars($nombre)."</strong>,<br><br>
        Hemos recibido tu solicitud. El barbero la revisará y recibirás un email de confirmación en breve.
      </p>
      <table style='width:100%;border-collapse:collapse;margin-bottom:28px;'>
        <tr><td style='padding:10px 0;border-bottom:1px solid #252530;color:#7a7880;font-size:13px;width:120px;'>Servicio</td>
            <td style='padding:10px 0;border-bottom:1px solid #252530;color:#f0ece3;font-size:13px;font-weight:600;'>".htmlspecialchars($servicioRow['nombre'])."</td></tr>
      ...
      <div style='background:rgba(201,168,76,.06);border:1px solid rgba(201,168,76,.2);border-radius:8px;padding:14px 16px;margin-bottom:24px;font-size:13px;color:#d4a84b;'>
        ⏳ Cuando el barbero confirme tu cita, recibirás otro email con el evento de calendario adjunto.
      </div>
      {$cancelBox}
    </div>
    <div style='background:#18181f;padding:16px 32px;text-align:center;'>
    ...
    jsonOk(['id' => (int)$id, 'mensaje' => $mensaje, 'estado' => $estadoFinal,
            'servicio' => $servicio, 'barbero' => $barbero, 'fecha' => $fecha, 'hora' => $hora]);

} catch (PDOException $e) {
    if ($e->getCode() === '23000') jsonError('Ese horario ya está reservado. Por favor elige otro.', 409);
...
function sendBrevo(string $toEmail, string $toName, string $subject, string $html): bool {
    ...
    $payload = json_encode(['sender' => ['name' => 'Prado Barber Co.', 'email' => 'endikapradodev@gmail.com'],
        'to' => [['email' => $toEmail, 'name' => $toName]], 'subject' => $subject, 'htmlContent' => $html]);
    ...
    $resp = curl_exec($ch); $httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE); $error = curl_error($ch); curl_close($ch);
    if ($httpCode !== 201) error_log("Brevo error [{$httpCode}] to:{$toEmail} curl:{$error} resp:{$resp}");
    return $httpCode === 201;
}